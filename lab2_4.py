# Программа читает текст из файла text.txt, находит:
# - Все email-адреса,
# - Номера телефонов в формате +7-XXX-XXX-XX-XX,
# - Слова, начинающиеся с заглавной буквы,
# и сохраняет результаты в файлы: emails.txt, phones.txt, capital_words.txt.
...
def main():
    ...
    # Записываем найденные email-адреса
    with open('emails.txt', 'w', encoding='utf-8') as f_emails:
        f_emails.write("\n".join(emails))
    # Записываем найденные номера телефонов
    with open('phones.txt', 'w', encoding='utf-8') as f_phones:
        f_phones.write("\n".join(phones))
    # Записываем найденные слова, начинающиеся с заглавной буквы
